...
Geometry::Geometry(std::vector<std::map<unsigned long,Detector>* >dets, double ver, Position tankc, double tankr, double tankhh, double mrdw, double mrdh, double mrdd, double mrds, int ntankpmts, int nmrdpmts, int nvetopmts, int nlappds, int statin){
	Detectors=dets;
	Version=ver;
	Status=statin;
	tank_centre=tankc;
...
Detector* Geometry::ChannelToDetector(unsigned long ChannelKey){
	...
	if(ChannelMap.count(ChannelKey)) return ChannelMap[ChannelKey];
	return 0;
...
Channel* Geometry::GetChannel(unsigned long ChannelKey){
	...
	if(ChannelMap.size()==0) InitChannelMap();
	if(ChannelMap.count(ChannelKey)) return &(ChannelMap[ChannelKey]->GetChannels()[ChannelKey]);
	return 0;
	...
	for(int i=0 ; i<Detectors.size(); i++){
		for (std::map<unsigned long,Detector>::iterator it=Detectors.at(i)->begin(); it!=Detectors.at(i)->end(); ++it){
			for(std::map<unsigned long,Channel>::iterator it2=it->second.GetChannels().begin(); it2!=it->second.GetChannels().end(); ++it2){
					...
					ChannelMap[it2->first]=&(it->second);
