...
// sparseMatrix.h -  CCS sparse nnlsmatrix class (double precision)
...
class sparseMatrix : public nnlsmatrix {
private:
double* data;
size_t* ridx;
size_t* cols;
...
sparseMatrix (size_t r, size_t c, size_t nnz) : nnlsmatrix(r, c) {
  assert (r > 0 && c > 0);
...
sparseMatrix(size_t r, size_t c, size_t nnz, size_t* ridx, size_t* cptr, double* val) : nnlsmatrix(r, c)
...
int set_row(size_t r, nnlsvector*&) { assert("Unsupported"); return -1;}

/// Sets the specified col to the given nnlsvector
int set_col(size_t c, nnlsvector*&) { assert("Unsupported"); return -1;}

/// Sets the specified diagonal to the given nnlsvector
int set_diag(bool p, nnlsvector*&) { assert("Unsupported"); return -1;}
