...
bool MonitorTankTime::Execute(){
  ...
  if(duration>=period_update){
    ...
    update_mins=true;
  	j_fivemin++;
    last=current;
    MonitorTankTime::UpdateMonitorSources();
    data_available = false;
...
void MonitorTankTime::InitializeVectors(){

  //-------------------------------------------------------
  //-----------------InitializeVectors---------------------
  //-------------------------------------------------------

  if (verbosity > 2) std::cout <<"MonitorTankTime: Initialize Vectors..."<<std::endl;
...
void MonitorTankTime::MRDTimePlots(){
    ...
    if (update_mins){
      if (draw_average){
        for (int i_mins=0;i_mins<num_fivemin;i_mins++){
          if (verbosity > 3) std::cout <<"MonitorTankTime: Stored data (1 hour): Channel #: "<<i_channel<<", bin "<<i_mins<<", time: "<<label_fivemin[i_mins].GetTime()<<", tdc: "<<times_channel_hour.at(i_channel)[i_mins]<<std::endl;
    ...
    if (draw_average){
      if (update_halfhour){
    	 for (int i_halfhour=0;i_halfhour<num_halfhour;i_halfhour++){
          if (verbosity > 3) std::cout <<"MonitorTankTime: Stored data (6 hours): bin "<<i_halfhour<<", time: "<<label_halfhour[i_halfhour].GetTime()<<", TDC: "<<times_channel_sixhour.at(i_channel)[i_halfhour]<<std::endl;
      ...
      if (update_hour){
        for (int i_hour=0;i_hour<num_hour;i_hour++){
          if (verbosity > 3) std::cout <<"MonitorTankTime: Stored data (day): bin "<<i_hour<<", time: "<<label_hour[i_hour].GetTime()<<", TDC: "<<times_channel_day.at(i_channel)[i_hour]<<std::endl;
...
void MonitorTankTime::UpdateMonitorSources(){
  ...
  //---------------------------------------------------
	...
	MonitorTankTime::MRDTimePlots();

}

void MonitorTankTime::FillEvents(){
  ...
  if (verbosity > 1) std::cout <<"MonitorTankTime: FillEvents..."<<std::endl;
  t_file_end = 0;
  int total_number_entries;  
  MRDdata->Header->Get("TotalEntries",total_number_entries);
  if (verbosity > 1) std::cout <<"MonitorTankTime: MRDdata number of events: "<<total_number_entries<<std::endl;
  int count=0;
