...
LAPPDBaseline::LAPPDBaseline() : Tool() {}

bool LAPPDBaseline::Initialise(std::string configfile, DataModel &data)
{
    /////////////////// Usefull header ///////////////////////
    if (configfile != "")
    ...
    execludeAmpDifference = 100;
    m_variables.Get("execludeAmpDifference", execludeAmpDifference);

    return true;
}

bool LAPPDBaseline::Execute()
{
    ...
    {
        if (BLSVerbosityLevel > 1)
            cout << "YES LAPPD Data found in baselinesub" << endl;
    }

    bool isBLsub = true;
    m_data->Stores["ANNIEEvent"]->Set("isBLsubtracted", isBLsub);
    if (BLSVerbosityLevel > 2)
        cout << "Made it to here " << BLSInputWavLabel << " is read in." << endl;
        ...
        for (int i = 0; i < Vwavs.size(); i++)
        {
            ...
            if (channelno == (LAPPDchannelOffset + (30 * bi) + TrigChannel))
            {
                for (int j = TrigLowBLfitrange; j < TrigHiBLfitrange; j++)
                {
                    if (j > 0)
                    {
                        if (abs(bwav.GetSamples()->at(j) - bwav.GetSamples()->at(j - 1)) < execludeAmpDifference)
            ...
            double AvgBL = BLval / ((double)(HiBLfitrange - LowBLfitrange - execuldedBins));
    ...
    if (BLSVerbosityLevel > 2)
        cout << "Baseline substraction is done in " << BLSOutputWavLabel << endl;
    m_data->Stores["ANNIEEvent"]->Set(BLSOutputWavLabel, blsublappddata);

    return true;
}

Waveform<double> LAPPDBaseline::SubtractSine(Waveform<double> iwav)
{
