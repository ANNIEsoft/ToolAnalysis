	...
	for (int i_slot = 0; i_slot < 36; i_slot++){
		...
		for (int i_channel =0; i_channel < num_channels_tank; i_channel++){
			for (int i_buffer = 0; i_buffer < BufferSize; i_buffer++){
				...
				//std::cout <<"random decision: "<<random_decision<<std::endl;
				double random_value;
...
bool MonitorTankLive::Finalise(){
    ...
    std::cout <<"Delete slot-wise canvasses"<<std::endl;
    std::cout <<"temp size: "<<canvas_Channels_temp.size()<<", freq size: "<<canvas_Channels_freq.size()<<std::endl;
    for (int i_slot = 0; i_slot < num_active_slots; i_slot++){
...
*/
  ...
  h2D_sigmatime = new TH2F("h2D_sigmatime","sigmatime",num_slots_tank,0,num_slots_tank,num_crates_tank*num_channels_tank,0,num_crates_tank*num_channels_tank);   //Time evolution of fitted sigma values of all PMT channels (in percent)
	...
	for (int i_active = 0; i_active<num_active_slots; i_active++){
	    ...
	    //std::cout <<"crate: "<<crate_num<<", slot: "<<slot_num<<std::endl;
		...
		//TCanvas *canvas_temp = new TCanvas("canvas_temp","Temp Canvas",900,600);
		...
		for (int i_channel = 0; i_channel < num_channels_tank; i_channel++){
	    	...
	    	//std::cout <<"i_channel: "<<i_channel<<", ss_title_hist: "<<ss_title_hist.str()<<std::endl;
...
void MonitorTankLive::TankPlots(){
  ...
  for (int i_data=0; i_data < CrateID.size(); i_data++){
	...
	//std::cout <<"hChannels_temp size: "<<hChannels_temp.size()<<", index: "<<i_active*num_channels_tank<<std::endl;
	...
	for (int i_channel = 0; i_channel < num_channels_tank; i_channel++){
		...
		//std::cout <<"Fit loop, i_channel: "<<i_channel<<", i_loop: "<<i_loop<<std::endl;
		//std::cout <<"hChannels_freq.size: "<<hChannels_freq.size()<<", hChannels_temp.size(): "<<hChannels_temp.size()<<std::endl;
		//std::cout <<"channels_mean.size: "<<channels_mean.size()<<", "<<channels_sigma.size()<<std::endl;

		//compute & fill the frequency plots
		long sum=0;
		for (int i_buffer = 0; i_buffer < BufferSize; i_buffer++){
			//std::cout <<"index for Data vector: "<<i_active*num_channels_tank*BufferSize+i_channel*BufferSize+i_buffer<<", Data size: "<<Data.size()<<std::endl;
     ...
     // std::cout <<"Setting bin content (temp): "<<(Data.at(i_active*num_channels_tank*BufferSize+i_channel*BufferSize+i_buffer*2)-offset)*conversion<<std::endl;
		...
		//std::cout <<"Filling 2D plots: mean = "<<channels_mean.at(i_loop)<<", sigma= "<<channels_sigma.at(i_loop)<<", sum= "<<sum<<", x= "<<x<<", y="<<y<<std::endl;
		...
		//std::cout <<"max_ped= "<<max_ped<<", min_ped= "<<min_ped<<", max_sigma= "<<max_sigma<<", min_sigma= "<<min_sigma<<", max_pedtime= "<<max_pedtime<<std::endl;
	...
	//std::cout <<"Drawing 2D histograms... "<<std::endl;
 	...
 	h2D_ped->Draw("colz");
   	...
   	for (int i_slot = 0; i_slot < num_slots_tank*num_crates_tank; i_slot++){
   		...
   		//std::cout <<"i_slot: "<<i_slot<<" / "<<num_slots_tank<<", slot_in_crate: "<<slot_in_crate<<std::endl;
	...
	if (h2D_ped->GetMaximum()>0.){
      ...
      (TPaletteAxis*)h2D_ped->GetListOfFunctions()->FindObject("palette");
        ...
        //h2D_ped->GetZaxis()->SetTitle("Pedestal");
      }
    p->Update();
	...
	if (h2D_sigma->GetMaximum()>0.){
      ...
      (TPaletteAxis*)h2D_sigma->GetListOfFunctions()->FindObject("palette");
        ...
        palette->SetY1NDC(0.1);
        palette->SetY2NDC(0.9);
        //h2D_sigma->GetZaxis()->SetTitle("Pedestal Sigma");
      }
      p_sigma->Update();
	...
	if (h2D_rate->GetMaximum()>0.){
      ...
      (TPaletteAxis*)h2D_rate->GetListOfFunctions()->FindObject("palette");
        ...
        palette->SetY1NDC(0.1);
        palette->SetY2NDC(0.9);
        //h2D_rate->GetZaxis()->SetTitle("Signal counts");
	}
	p_rate->Update();
    ...
    double min_temp = 999999.;
		...
		for (int i_channel = 0; i_channel < num_channels_tank; i_channel++){
			//std::cout <<"Temp i_channel: "<<i_channel<<", ";
      ...
      leg->AddEntry(hChannels_temp.at(i_slot*num_channels_tank+i_channel),ss_channel.str().c_str(),"l");
		...
		delete leg;
		//TList *list_temp = (TList*) canvas_Channels_temp.at(i_slot)->GetListOfPrimitives();
    ...
    //std::cout <<"max temp: "<<max_temp<<std::endl;
		...
		for (int i_channel = 0; i_channel < num_channels_tank; i_channel++){
			//std::cout <<"Freq i_channel: "<<i_channel<<", ";
			std::stringstream ss_channel;
			ss_channel << "Cr "<<crate_num<<"/Sl"<<slot_num<<"/Ch"<<i_channel;
			hChannels_freq.at(i_slot*num_channels_tank+i_channel)->Draw("same");
			//std::cout <<"Freq title: "<<hChannels_freq.at(i_slot*num_channels_tank+i_channel)->GetTitle()<<std::endl;
